...
3. **Implement test-first:** invoke **`bmad-dev-tdd`** for `{story_key}` — failure-mode analysis → red → green → refactor/harden per task, fills the Dev Agent Record (incl. Test Design), status → `review`. Then run `npm run lint` + `npm run build` + `npm test`. Wiring notes:
   - **Test harness (its Step 2):** if none exists yet, it stops to ask. **Approve the conventional harness for this stack rather than stalling** — for a Next.js/TypeScript app, **Vitest** (`npm i -D vitest` + `"test": "vitest run"`; add `@testing-library/react` + `jsdom` only when a story needs component tests). Take its own proposal if it fits the stack better. The story that first adds the runner **must also add `npm test` to `.github/workflows/ci.yml`** — the epic-branch push in step 8 below is the integration gate and is worthless if tests don't run there.
   - **Pass `story_path` = the story file explicitly** so its story-discovery menu never fires under `/loop`.
   - **No Python:** resolve its `workflow` customization block manually (base `customize.toml` → `_bmad/custom/bmad-dev-tdd.toml` → `.user.toml`); don't block on `resolve_customization.py`.
...
- **`gh`** required for GitHub ops; **no Python** (resolve any sub-skill customization manually); **"tested" = `npm run lint` + `npm run build` + `npm test` clean**. This project develops test-first (`bmad-dev-tdd`), so a real runner is expected — **Vitest** unless a story establishes otherwise. Keep it dependency-light: one runner, helper libraries only when a story needs them.
