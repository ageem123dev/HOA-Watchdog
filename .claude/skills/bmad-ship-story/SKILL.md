...
- **Test harness (its Step 2).** If no test harness exists yet, `bmad-dev-tdd` stops and asks to set one up. **Under `/loop`, approve the conventional harness for this stack rather than stalling** — for a Next.js/TypeScript app that means **Vitest** (`npm i -D vitest` + a `"test": "vitest run"` script; add `@testing-library/react` + `jsdom` only when a story actually needs component tests). Commit the harness setup with the story. If it proposes something else that fits the stack better, take its proposal. Only STOP if it wants a heavyweight runtime change (a new framework, a DB-backed test env) that the story doesn't cover.
- **First story that adds the harness must also add `npm test` to CI** (`.github/workflows/ci.yml`, alongside lint and build) — otherwise CI stays green on broken tests and Step 7 is meaningless.
- **No Python:** its Step 1 and Step 11 call `resolve_customization.py`. That script won't run here — resolve the `workflow` block manually (base `customize.toml` → `_bmad/custom/bmad-dev-tdd.toml` → `.user.toml`) as its own fallback instructions describe.
...
- **"Tested" = `npm run lint` + `npm run build` + `npm test` clean.** This project develops test-first (`bmad-dev-tdd`), so a real test runner is expected — **Vitest** unless a story establishes otherwise. Keep it dependency-light: one runner, add helper libraries (`@testing-library/react`, `jsdom`, MSW) only when a story genuinely needs them. Whichever story first introduces the runner also adds `npm test` to CI so all three gates run there.
