...
2. Ask the user for **CodeRabbit → Start Review**, base `main`, scope **committed changes**. The extension bases on local `main`, so fast-forward it first (Section 2 does) or step 4's SHA check fails against `origin/main`. It is started by hand and does **not** re-trigger on a push; ask once per round and STOP until it finishes (~11 min for a 3-file diff).
...
4. **Accept it only if `status` is `completed` AND `headCommitId` == `git rev-parse HEAD` AND `baseCommitId` == `git rev-parse main`.** Both stored values are 40-char SHAs, not ref names. Compare against *local* `main` — the base the extension actually used — with item 2's fast-forward keeping that honest. Comparing against `origin/main` instead rejects a perfectly good review whenever someone merges upstream mid-round. The key alone also matches a clean review taken before your last fix commit, which would pass unreviewed code as converged — 8c's precondition, in a new place.
...
2. Else write the description to a scratch file `{description_file}` and run `glab mr create --source-branch {branch} --target-branch main --title "{story_id}: {title}" --description "$(cat {description_file})" --yes`. **The body must come from a file** — backticks in a double-quoted bash string get command-substituted. `--title` is exposed the same way and is not file-backed: strip or escape any backtick or `$` in `{title}` before interpolating it. `{description_file}` is a scratch path you choose, not `story_file` and not a literal `file`.
