...
- **Rule:** The Node gateway reaches the Python agent service through **versioned `/chat/v*` endpoints only**. A chat turn carries a **question and an answer** — never rows, never SQL, and never a catalog entry id supplied by the caller. The agent obtains every fact for itself through AD-15's `/tools/v1/*`, so this endpoint adds no route to the association's records and is not a data path. Caller identity is asserted by a shared service token held only by the gateway and the agent service, **distinct from `AGENT_SERVICE_TOKEN`**: one token reused in both directions means either runtime's compromise grants the other's identity.
