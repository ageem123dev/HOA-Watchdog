...
- **Amendment (2026-08-21, before any code).** The token above authenticates a **runtime**, and this rule should not be read as authenticating a **person**. `/tools/v1/catalog/execute` takes an `actorId`, and since story 5.1b the association whose records are returned is derived from it — so the token now stands in front of a decision it says nothing about. Anything holding it may name any board member. **AD-18 governs that**, and this rule is unchanged: the service token remains what proves the caller is the agent service, and nothing more.
...
- **Amendment (2026-08-21, before any code).** The request carries a question **and an `actorId`**, which the original wording did not mention because at the time the actor only identified who to attribute a provenance row to. Since story 5.1b it also determines *whose records the turn may read*, and it makes a round trip through the agent service before returning to Node at `/tools/v1/*`. The rule that the request carries no SQL, no rows and no entry id is unchanged and remains the point of this AD. What it never claimed, and must not be read as claiming, is that the `actorId` coming back is the one that went out. **AD-18 governs that**, and the actor assertion it introduces travels this same wire.

### AD-18 — The actor is proved to the gateway, not asserted by its caller

- **Decided:** 2026-08-21, after story 5.1b made the association a derived value and MR !71 raised the consequence.
- **Binds:** AD-5, AD-12, AD-15, AD-17, `/tools/v1/*`, the `NEXT → PY → NEXT` round trip
- **Prevents:** A holder of a service token reading any association's records by naming a member of it; a provenance row that says a named director asked a question when nothing establishes that they did.
- **Rule:** A `/tools/v1/*` request establishes **which board member it is for** by presenting a short-lived assertion **minted by the Node gateway and verified by the Node gateway**. The agent service **relays it opaquely and holds no signing key**. The association is derived from the verified subject; an `actorId` supplied as an ordinary request field is not evidence and must not be believed.

- **What the existing two credentials do not cover.** AD-15's and AD-17's tokens authenticate **runtimes**: they establish that a caller is the agent service, or is the gateway. Neither carries a **subject**. Both are long-lived, symmetric and shared by every turn, so nothing in either distinguishes "the director asked this" from "something holding the token said they did". `actorId` rides inside those authenticated channels as ordinary data, and an authenticated channel carrying an unauthenticated claim is exactly the shape this rule exists to name.

- **Why Node signs something it verifies itself.** The obvious objection, and the answer is the round trip: the value leaves Node's trust boundary, passes through a runtime holding a model credential and processing untrusted document text, and comes back. Signing is what makes the return leg checkable. This is not a credential shared with another party — the agent service can neither mint nor inspect it — which is why it is a *third* credential rather than a third use of the two that exist.

- **What this does not claim.** It does not make the agent service trusted, and it is not row-level security. A compromised agent can still relay a **legitimate** assertion for the member whose turn it is, and read that member's association — which is the access that member already has. It bounds the blast radius to one subject per turn; it does not bound what that subject may see. AD-4's amendment still calls onboarding a second association without RLS a defect rather than a trade-off, and this rule does not change that.

- **Why now, and what it is not a response to.** Story 5.1b derived the association from `actorId`, which moved the trust anchor onto it. A review read that as a prompt-injected model being able to choose the actor. **It cannot** — `route_question` takes the actor as a keyword argument, the model returns only an entry name and its arguments, and no entry may declare a parameter naming an actor (`core/security/actor-is-never-chosen.test.ts` pins all three). Recording the refuted mechanism matters because designing against it would put the control **inside the agent**, which is the component that would then have to be trusted to apply it.

- **What it costs:** a signing key on the Node side, a per-turn mint and verify, and an expiry window short enough to bound replay but long enough that a slow model call does not fail a legitimate turn. The agent service gains one opaque field to carry and no new secret.
