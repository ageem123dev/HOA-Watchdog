...
- **Multi-tenancy.** ~~The pilot is one association. Row-level security, tenant scoping in the catalog, and per-tenant vendor tables are deliberately out of scope.~~ **Partly resolved 2026-08-18** (AD-4 and AD-5 amendments): epic 5 introduces the association entity and makes catalog scoping a rule enforced by test, because import mappings belong to an association and the entity could not wait. **Still deferred and now the whole of the gap: row-level security, and per-tenant vendor tables.** The pilot remains one association; a second may not be onboarded until RLS exists.
